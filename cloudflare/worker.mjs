...
async function load(env, email) {
  ...
  if (!row) {
    ...
    if (email !== ownerEmail) throw new ApiError(403, 'Huvudtränaren behöver öppna appen först.');
    const id = crypto.randomUUID(), state = Store.initial(id, env.OWNER_NAME || 'Huvudtränare');
    state.coaches[0].email = ownerEmail;
    const data = { state, ownerId: id };
...
export function createHandler(verify = verifyIdentity) {
  return {
    async fetch(request, env) {
      ...
      if (url.pathname === '/config.js' && request.method === 'GET') return new Response('window.LAGBYGGAREN_CONFIG = { shared: true, auth: "access", pollMs: 30000 };', { headers: { ...headers, 'Content-Type': 'text/javascript; charset=utf-8' } });
      ...
      try {
        ...
        const data = await load(env, email);
        const actor = data.state.coaches.find(c => c.email === email)?.id;
        ...
        if (saved.meta.changes !== 1) {
          const current = await load(env, email);
          if (!current.state.coaches.some(c => c.id === actor && c.email === email)) throw new ApiError(403, 'Din åtkomst har tagits bort.');
