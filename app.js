...
function teamsHTML(p, movable = false, staffEditable = false) {
  return `<p class="hint">${esc(p.kind)}${p.date ? ' · ' + esc(p.date) : ''} · ${p.slots.length - 1} + 1 · ${p.mode === 'balanced' ? 'Jämnstarka' : 'Nivåanpassade'} lag · Snitt ${num(TeamEngine.average(p.teams.flat()))}</p><div class="teams">${p.teams.map((team, i) => {
    ...
    return `<article class="team"><div class="teamhead"><h3>Lag ${i + 1}</h3><p>${team.length} spelare · Snittnivå <b>${num(TeamEngine.average(team))}</b>${p.mode === 'tiered' ? ' · Mål ' + num(p.targets[i]) : ''}</p></div><div class="teamstaff">${staffEditable ? `<label>Tränare · Lag ${i + 1}<input data-team-detail data-team-coach="${i}" maxlength="200" value="${esc(p.teamCoaches?.[i] || '')}" placeholder="Till exempel Håkan och Anna"></label><label>Samlingstid · Lag ${i + 1}<input data-team-detail data-team-time="${i}" type="time" value="${esc(p.teamMeetings?.[i]?.time || '')}"></label><label>Plats · Lag ${i + 1}<input data-team-detail data-team-place="${i}" maxlength="200" value="${esc(p.teamMeetings?.[i]?.place || '')}" placeholder="Till exempel entrén till Lödde idrottshall"></label>` : `${p.teamCoaches?.[i] ? `<p><b>Tränare:</b> ${esc(p.teamCoaches[i])}</p>` : ''}${p.teamMeetings?.[i]?.time ? `<p><b>Samling:</b> ${esc(p.teamMeetings[i].time)}</p>` : ''}${p.teamMeetings?.[i]?.place ? `<p><b>Plats:</b> ${esc(p.teamMeetings[i].place)}</p>` : ''}`}</div><div class="teamlist">${missing.length ? `<p class="warning">Saknar positionstäckning: ${missing.join(', ')}. Kontrollera uppställningen.</p>` : ''}${rows}${bench.length ? '<p class="sub">AVBYTARE · ' + bench.length + '</p>' + bench.map(x => row(x, i, x.first, '', movable)).join('') : ''}</div></article>`;
  }).join('')}</div>`;
}
...
function showReview(id, refresh = false) {
  ...
  const previousStaff = refresh && $('#reviewdialog').dataset.revision === String(p.revision) ? Array.from(document.querySelectorAll('[data-team-detail]'), el => el.value) : null;
